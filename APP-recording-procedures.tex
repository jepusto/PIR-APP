...
Following a derivation in \citet{Brown1975estimation}, the expected information for MTS data is given by  
\begin{equation}
\label{eq:MTS_Info}
\begin{aligned}
...
\info^{M}_{\phi\rho} &= K_M \frac{c_M (1 - 2 \phi)\exp(- 2 \rho c_M)}{\left[1 - p_0(c_M)\right]p_1(c_M)}\\
\info^{M}_{\rho\rho} &= K_M \frac{\phi(1 - \phi)c_M^2 \exp(-2\rho c_M)}{1 - \exp(-\rho c_M)}\left[\frac{1}{1 - p_0(c_M)} + \frac{1}{p_1(c_M)}\right]
\end{aligned}
\end{equation}

\todo[inline]{Expected information matrix for MTS.}
