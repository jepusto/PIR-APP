...
\end{equation}
\todo[inline]{Any other comments on MTS?}
...
\todo[inline]{Comment on expected information for PIR}
...
\todo[inline]{Comment on expected information for AIR}
...
\end{equation}
where the sum is taken over the transition probabilities that are strictly positive \citet{Billingsley1961statistical}.\todo{Does the same hold for countably infinite state spaces?}
...
\[
\Pr\left(Z(t_k) = 1, W_{k-1} = 0 \left| Z(t_{k-1}) = 1 \right.\right) = p_1(c + d) - p_1(d) \exp\left(-\rho(1 - \phi)c\right). \]
\end{lemma}
...
&= p_0(d)\exp\left(-\rho \phi c\right).
\end{align*}
Similarly,
\begin{align*}
\pi_{0|000} &= \Pr\left(X_k = 0, U_k = 0 | X_{k-1} = 0\right) = \left[1 - p_0(d)\right]\exp\left(-\rho \phi c\right) \\
\pi_{1|111} &= \Pr\left(X_k = 1, W_k = 1 | X_{k-1} = 1\right) = p_1(d)\exp\left(-\rho(1 - \phi)c\right) \\
\pi_{1|011} &= \Pr\left(X_k = 0, W_k = 1 | X_{k-1} = 1\right) = \left[1 - p_1(d)\right]\exp\left(-\rho(1 - \phi)c\right).
\end{align*}
Next, it follows from Lemmas \ref{lemma1} and \ref{lemma2} that
\begin{align*}
\pi_{0|110} &= \Pr\left(X_k = 1, U_k = 1 | X_{k-1} = 0\right) = p_0(c + d) - p_0(d) \exp\left(-\rho \phi c\right) \\
\pi_{1|110} &= \Pr\left(X_k = 1, W_k = 0 | X_{k-1} = 1\right) = p_1(c + d) - p_1(d) \exp\left(-\rho(1 - \phi)c\right).
\end{align*}
The two remaining transition probabilities can be obtained by subtraction:
\begin{align*}
\pi_{0|010} &= 1 - \pi_{0|000} - \pi_{0|100} - \pi_{0|110} = 1 - p_0(c + d) - \left[1 - p_0(d)\right]\exp\left(-\rho \phi c\right) \\
\pi_{1|010} &= 1 - \pi_{1|011} - \pi_{1|110} - \pi_{1|111} = 1 - p_1(c + d) - \left[1 - p_1(d)\right]\exp\left(-\rho(1 - \phi)c\right).
\end{align*}

The information matrix for the AIR model can be derived using Billingsley's formula. The equilibrium probability that $X_k = 1$ is $\phi$, by which it follows that the equilibrium distribution of the states is given by \[
\Pr(X_k = b, U_k = c, W_k = d) = (1 - \phi) \pi_{0|bcd} + \phi \phi_{1|bcd}, \]
for $b,c,d = 0,1$. The derivatives of the transition probabilities with respect to $\phi$ and $\rho$ are as follows:
\begin{align*}
\end{align*}
\end{document}