...
params <- subset(params, (k_priors > 1 | theta == Inf) & (theta < Inf | k_priors == 1))
with(params, table(k_priors, theta))
...
# execute simulations
system.time(results <- mdply(params, .fun = runSim, c = 1, rest_length = 0, iterations = 5000, .parallel=TRUE))
stopCluster(cluster)

save(results, file = "PIR hyperparameter sims.Rdata")
