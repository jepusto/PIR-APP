...
PIR_wrap <- function(data_line, c = 1, d = 0, k_prior = 1, theta_prior = Inf){
  ...
  results <- PIRbootstrap(U = U, c = c, d = d, coding = data_line[3], 
         penalty_func = Beta_Gamma(k_mu = k_prior, k_lambda = k_prior, 
                                   ...
                                   theta_lambda = theta_prior), transform = "exp", iterations = 101)
...
mean_sq_error <- ddply(phi, .(Measurement.Type), summarize, MSE = sum(sq_error)/16, MSEsum = sum(sq_errorSum)/16)
kable(mean_sq_error, col.names = c("Measurement Type", "PLE", "Summary"), caption = "Error", digits = 4)
```

Table 1 contains the value of the "mean squared error" of both the PLEs and the summary measurements. In this case we define "mean squared error" as $\frac{\sum_{i = 1}^{n} (\hat\theta - CDR)^2}{n}$, where $\hat\theta$ is a given estimate of prevalence. The error for PIR PLE is about twice that of the summary measurement, while the error for WIR PLE is very small compared to the summary measurement. This may be an issue of the model not accounting for the slightly different method of PIR used in this data, whereas our model is correct for the WIR data.
