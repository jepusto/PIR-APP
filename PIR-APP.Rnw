...
The simulation study reported in the next section therefore uses the Nelder-Mead algorithm \citep{Nelder1965simplex}, which does not require evaluation of the score function.
...
\end{equation}
Given data based on recording system $s \in \{MTS, PIR, WIR\}$, the PLEs $\tilde\phi_s, \tilde\zeta_s$ are defined as the values that maximize $l_s\left(\phi, \zeta\right) + p\left(\phi, \zeta\right)$. 
...
For MTS data, the summary proportion is an unbiased estimate of prevalence under a very broad class of data-generating models \citep{Rogosa1991statistical}; however, the summary proportion may be less efficient than the PLE or MLE for prevalence under the Alternating Poisson Process model. It is therefore important to compare the accuracy of all of these estimators.
...
However, for a fixed sample size, it remains to be seen whether likelihood-based estimates of prevalence are more accurate than the PIR summary proportion. 

In order to understand the operating characteristics of the MLEs and PLEs in samples with a finite number of intervals, we conducted a computer simulation study. The simulations examined two specific questions: First, for a given type of data (MTS or PIR), how does the accuracy of the PLEs compare to the accuracy of the MLEs (for both prevalence and incidence) and the accuracy of the summary proportion (for prevalence only)? Second, how large a sample is needed to obtain approximately unbiased estimates of prevalence or incidence using penalized likelihood methods?

The simulation design is summarized in Table \ref{tb:simulation_design}.
For all simulation conditions, we set $c = 1$ and $d = 0$, so that the values of incidence are scaled in terms of the length of an interval; for example, $\zeta = 0.1$ corresponds to an incidence of one new behavioral episode per ten intervals. We chose prevalence values that spanned nearly the entire possible range because PIR might be used to observe a behavior that occurred many different proportions of the time. We chose to range between an incidence of $\zeta = .05 and \zeta = .50$ because these represented a behavior that was occurring from on average once every 20 intervals to once every 2 intervals. If the behavior occurred any more infrequently and it was unlikely to have occurred even once during the observation for many of the values of $K$, and would be potentially so infrequent that it might not be observed by MTS. If the behavior occurred any more frequently, then the hypothetical interval length chosen was probably inappropriate for the behavior and likely requires tuning to effectively capture the behavior. The number of intervals might represent anything from a very quick observation in a classroom where the hypothetical observer needed to capture the behaviors of several children in a short time span, to an intensive observation of a child over the course of an entire class period.

We simulated both PIR and MTS measurements using the ARPobservation package for the R statistical computing environment. For each set of conditions and set of hyperparameters (either $\alpha_\mu = \alpha_\lambda = \Sexpr{alpha_default}$ and $\theta_\mu = \theta_\lambda = \Sexpr{theta_default}$ or $\alpha_\mu = \alpha_\lambda = 1$ and $\theta_\mu = \theta_\lambda = \infty$), we generated 10000 behavior streams of length $K$ with mean event duration $\mu = \phi/\zeta$ and mean interim time of $\lambda = (1-\phi)/\zeta$. We applied the appropriate observation method with $K$ intervals which yielded a binary response string of length $K$. For each response string we calculated the proportion of intervals containing a 1 and then used numerical maximization to find the set of estimators $\hat\phi, \hat\zeta$ that maximized $l_s\left(\phi, \zeta\right) + p\left(\phi, \zeta\right)$. When $\alpha_\mu = \alpha_\lambda = 1$ and $\theta_\mu = \theta_\lambda = \infty$, $p\left(\phi, \zeta\right)$ reduces to zero and the function maximized simplifies to equation (4) for MTS and equation (8) for PIR.

We examined the performance of the estimators for four different parameterizations. We were interested in both the natural parameterizations of $\phi$ and $\zeta$, as well as the logit of $\phi$, $log\left(\phi / (1 - \phi)\right)$, and $log(\zeta)$. In the natural parameterization of $\phi$, the logit of $\phi$, and the log of $\zeta$  we examined bias, $\E\left[\hat\theta - \theta\right]$, and root mean squared error (RMSE), $\sqrt{Var(\hat\theta) + (Bias(\hat\theta))^2}$, where $\theta$ is the parameter of interest. In the natural parameterization of $\zeta$ we examined the relative bias and the relative RMSE: \[ Bias_{rel}(\zeta) = \frac{\E\left(\hat\zeta - \zeta\right)}{\zeta}, \qquad RMSE_{rel}(\zeta) = \frac{\sqrt{Var(\hat\zeta) + (Bias(\hat\zeta))^2}}{\zeta}.\] 

\todo[inline, color=green!40]{I feel like probably I have made an inadvertent mistake from lack of knowledge in here and left out a hat or failed to include one so please check carefully}

When describing the estimators, have set varying criteria for what we believe constitutes ``approximately unbiased.'' In the case of $\phi$ this is an absolute bias less than 0.03 and in the case of $logit(\phi)$ an absolute bias less than 0.05. Lacking other estimators for incidence, we have set rather liberal criteria in the cases of $\zeta$ and $log(\zeta)$. In the case of $\zeta$ it is a relative absolute bias of less than 0.10 and in the case of $log(\zeta)$ an absolute bias of less than 0.10. \todo[color=green!40]{Why these criteria? Is "approximately unbiased" actually what I should be saying here??} 
\todo[inline]{Describe and justify design.}
\todo[inline]{Define performance measures: bias/RMSE of $\phi$, relative bias/RMSE of $\zeta$, bias/RMSE of $\logit \phi, \log \zeta$.}
...
$K$ & Number of intervals & 15 & 10 & 10 & 150 \\
$s$ & Recording system & 2 & \multicolumn{3}{c}{MTS, PIR} \\
\bottomrule
\end{tabular}
\end{table}
...
Figure \ref{fig:MTS_RMSE} illustrates the distribution of RMSE of the MTS MLEs, PLEs, and proportions for each parameterization across varying levels of $K$. Given that the MTS proportions are known to be unbiased estimates of prevalence, those estimates provide a point of reference for the accuracy of the MLEs and PLEs. Across all levels of $K$, the MLEs of $\phi$ have comparable RMSEs to the MTS proportions. However, the MLEs perform notably worse in terms of $logit(\phi)$ until $K = 120$ or more, a very large number of intervals. Across all levels of $K$, the PLEs of $\phi$ and $logit(\phi)$ have RMSEs comparable to or smaller than the MTS proportions. In terms of both $\zeta$ and $log(\zeta)$, the MLEs perform considerably worse across all levels of $K$ than the PLEs and the corresponding estimates of prevalence. In contrast, the PLEs perform quite well for both $\zeta$ and $log(\zeta)$, with accuracy comparable to the accuracy of the corresponding estimates of prevalence.
<<MTS_RMSE, fig.width = 10, fig.height = 7, fig.cap="Distribution of root mean-squared error of MLE, PLE, and summary proportion (Prop.) estimates based on MTS data, across range of $0.05 \\leq \\phi \\leq 0.95$ and $0.05 \\leq \\zeta \\leq 0.50$. Values above 1.5 are not displayed. For zeta, the root mean-squared error is relative to the true value.">>=
...
Figure \ref{fig:MTS_sample_size} illustrates the minimum value of $K$ needed to meet the criteria we have set for ``approximately unbiased'' when utilizing the penalized estimators. In terms of $\phi$ and $logit(\phi)$, the estimator requires at most 40 intervals when $\zeta/c \geq 0.10$ across the entire range of $\phi$. Even when $\zeta/c$ drops to 0.05, unbiased estimates of phi can be obtained across the entire range if $K = 100$ and unbiased estimates of $logit(\phi)$ can be obtained when $K = 60$. In terms of $\zeta$, unbiased estimates can be obtained when $K$ is at least 60 for $0.30 \leq \phi \leq 0.70$ and $\zeta \geq 0.10$. Unbiased estimates can also be obtained when $K$ is at least 70 for $0.05 \leq \phi \leq 0.95$ and $0.15 \leq \zeta \leq 0.25$. In terms of $log(\zeta)$, unbiased estimates can be obtained when $K$ is at least 70 and $0.40 \leq \phi \leq 0.60$ and $\zeta \geq 0.15$. Unbiased estimates can also be obtained when $K$ is at least 60 and $0.15 \leq \phi \leq 0.85$ and $0.10 \leq \zeta \leq 0.25$.
<<MTS_sample_size, fig.width = 10, fig.height = 8, fig.cap="Minimum number of MTS intervals required to obtain PLEs with absolute bias less than given criteria.">>=
...
Figure \ref{fig:PIR_RMSE} illustrates the distribution of RMSE of the PIR MLEs, PLEs, and proportions for each parameterization across varying levels of $K$. In contrast to MTS, we expect that the proportion estimates of prevalence for PIR will perform quite poorly. In fact, this is the case. In terms of the natural parameterization of $\phi$, the MLE performs notably better than the proportions when $K \geq 60$, while the PLE performs better across the entire range of $K$. In terms of $logit(\phi)$, both the MLE and the proportion perform worse than the PLE. In both parameterizations of $\phi$, the PLE performs just slightly worse for the PIR estimates than it did for the MTS estimates. In terms of the natural parameterization of $\zeta$, the PLEs once again provide more accurate estimates of incidence. However, unlike with MTS data, the relative RMSE of the MLEs are not at ceiling even if the PLEs are clearly preferable. In terms of $log(\zeta)$, the PLEs are definitely preferable and the RMSE of the MLEs once again range all the way up to the ceiling of 1.5 The mean RMSE of the PIR PLEs is similar to the MTS estimates of incidence for both parameterizations, but the spread when using PIR is larger.
<<PIR_RMSE, fig.width = 10, fig.height = 7, fig.cap="Distribution of root mean-squared error of MLE, PLE, and summary proportion (Prop.) estimates based on PIR data, across range of $0.05 \\leq \\phi \\leq 0.95$ and $0.05 \\leq \\zeta \\leq 0.50$. Values above 1.5 are not displayed. For zeta, the root mean-squared error is relative to the true value.">>=
...
Figure \ref{fig:PIR_sample_size} illustrates the minimum value of $K$ needed to meet the criteria we have set for ``approximately unbiased'' when utilizing the penalized estimators. The portion of the parameter space where one can obtain approximately unbiased estimates of prevalence with a moderate number of intervals is much smaller than with the MTS estimates. In terms of the bias of $\phi$, unbiased estimates can be obtained when $K$ is at least 70 and $0.10 \leq \phi \leq 0.35$ and $\zeta/c \leq 0.4$. Unbiased estimates can also be obtained when $K$ is at least 60 and $\phi \leq 0.80$ and $\zeta/c \leq 0.25$. In terms of $logit(\phi)$, unbiased estimates can be obtained when $K$ is at least 70 and $0.15 \leq \phi \leq 0.35$ and $\zeta/c \leq 0.30$. Unbiased estimates can also be obtained when $K$ is at least 90 and $\phi \leq 0.70$ and $\zeta/c \leq 0.15$. In order to obtain unbiased estimates of incidence, much larger values of $K$ are required. In terms of $\zeta$, unbiased estimates can be obtained when $K$ is at least 130 and $0.10 \leq \phi \leq 0.35$ and $\zeta \leq 0.45$. Unbiased estimates can also be obtained when $K$ is at least 130, and $\phi \leq 0.60$ and $\zeta/c \leq 0.25$. In terms of $log(\zeta)$, unbiased estimates can be obtained when $K$ is at least 140 and $\phi \leq 0.40$ and $\zeta \leq 0.35$
<<PIR_sample_size, fig.width = 10, fig.height = 8, fig.cap="Minimum number of PIR intervals required to obtain PLEs with absolute bias less than given criteria.">>=
...
For each of the three recording procedures, 15 s intervals were used, yielding a total of 40 intervals per scored clip.
Each video clip was independently coded by ten different raters using each of the recording systems. 

The goal of our analysis is to estimate the prevalence and incidence of the behavior observed in each video clip, using the data collected using each of the three recording systems. 
Because each video was scored separately using each recording system, comparing the PLEs obtained from each type of data allows us to characterize the strengths and weaknesses of each recording system. The fact that each clip is scored by multiple raters also allows us to observe a source of measurement error that the Alternating Poisson Process model does capture: "inter-rater" error, due the human observers not perfectly following the scoring procedure or not perceiving the behavior stream with perfect accuracy.

<<summary_prop, fig.width = 8, fig.height = 5, fig.cap="MTS, PIR, and WIR summary proportions for each rater, by clip. Horizontal dashed lines correspond to the average MTS summary proportions.">>=
...
The quality of the estimates varies substantially by both recording procedure and by clip. For MTS, the maximum likelihood estimates are all close to the raw proportions (which are unbiased estimates of prevalence). The MTS confidence intervals cover the benchmark prevalence estimate in \Sexpr{sum(covered$MTS)} out of the 60 MTS records and are generally narrower than the confidence intervals based on the PIR and WIR data.
...
<<AIR_RMSE, fig.width = 10, fig.height = 7, fig.cap="Distribution of root mean-squared error of PLE estimates based on AIR, MTS, and PIR data, across range of $0.05 \\leq \\phi \\leq 0.95$ and $0.05 \\leq \\zeta \\leq 0.50$. Values above 1.5 are not displayed. For zeta, the root mean-squared error for is relative to the true value.">>=
