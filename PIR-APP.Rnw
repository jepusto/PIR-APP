...
We therefore use boundaries of $|\logit \ \hat\phi| < \Sexpr{threshold}$ and $|\log \hat\zeta| < \Sexpr{threshold}$ to define the edges of the parameter space. 
...
In this section, we describe two recording procedures that can be used to obtain more precise estimates of both prevalence and incidence. 
...
The sample space of $(U_k,W_k,X_k)$ contains four unique outcomes. 
For ease of notation, define the variables $Y_k = U_k + W_k + X_k$ for $k=1,...,K$; note that this is a unique mapping. 
The complete record of observed data is then $(X_0,Y_1,...,Y_K)$. 

The Markov property of the Alternating Poisson Process implies that the joint distribution can be written as \[
\Pr\left(X_0=x_0,Y_1 = y_1,...,Y_K = y_K\right) = \Pr\left(X_0 = x_0\right) \prod_{k=1}^K \Pr\left(Y_k = y_k | X_{k-1} = x_{k-1}\right). \]
Denote the transition probabilities $\Pr\left(Y_k = b | X_{k-1} = a\right) = \pi_{ab}$ and let $m_{ab} = \sum_{k=1}^K I\left(X_{k-1} = a, Y_k = b\right)$ for $a = 0,1$ and $b = 0,1,2,3$. 
The transition probabilities are given by the following functions of the behavior stream parameters:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Appendix \ref{app:AIR_derivation} provides the derivation of these quantities.
...
This appendix provides a derivation of Expression (\ref{eq:psi_k}) in terms of the parameters of the Alternating Poisson Process. 
Note that $U_{k-1} = 0$ implies that $Z(t_k + c) = 0$. 
...
This appendix provides a derivation of the transition probabilities for the AIR model in terms of the parameters of the Alternating Poisson Process. 
\begin{align*}
\pi_{00} &= \Pr\left(X_k = 0, U_k = 0 | X_{k-1} = 0\right) \\
&= \Pr\left(X_k = 0 | Z(t_k + c) = 0\right) \Pr\left(U_k = 0 | X_{k-1} = 0\right) \\
&= \left[1 - p_0(d)\right]\exp\left(\frac{- \zeta c}{1 - \phi}\right) \\
\pi_{01} &= \Pr\left(X_k = 1, U_k = 0 | X_{k-1} = 0\right) + \Pr\left(X_k = 0, U_k = 1 | X_{k-1} = 0\right) \\
&= p_0(d)\exp\left(\frac{- \zeta c}{1 - \phi}\right) + \left[1 - p_0(c + d)\right] - \left[1 - p_0(d)\right]\exp\left(\frac{- \zeta c}{1 - \phi}\right) \\
&= \left[1 - p_0(c + d)\right] - \left[1 - 2 p_0(d)\right]\exp\left(\frac{- \zeta c}{1 - \phi}\right) \\
\pi_{02} &= \Pr\left(X_k = 1, U_k = 1 | X_{k-1} = 0\right) \\
